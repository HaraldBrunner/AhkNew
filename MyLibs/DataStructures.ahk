...
evalPrim(v){
	...
	if v is integer
	{
		...
		WinGet, proc, ProcessName, ahk_id %v%
		WinGet, pid, PID, ahk_id %v%
		if (proc!=""){
			r:= v . " ("  . vhex  . ") " . proc . "  (" . pid . ")  " . title
		}
...
ObjToStr(Obj, Indent="`t", Rows="`n", Equal=" = ", Depth=7, CurIndent="",  evalPri=false) {	; converts object to string
    For k,v in Obj
        ToReturn .= CurIndent . k . (IsObject(v) && depth>1 ? Rows . ObjToStr(v, Indent, Rows, Equal, Depth-1, CurIndent . Indent,  evalPri) : Equal . ( evalPri ? evalPrim(v) : v ) ) . Rows
    return RTrim(ToReturn, Rows)
}	; http://www.autohotkey.com/forum/post-426623.html#426623

ObjToStrUser(Obj, Indent="`t", Rows="`n", Equal=" = ", Depth=7, CurIndent="",  evalPri:=false) {
	return ObjToStr(Obj, Indent, Rows, Equal, Depth, CurIndent,true)
}
